        ...
        try {
            ...
            if ("bpms_instance_change".equals(eventType)) {
                if ("start".equals(type)){
                    log.info("----审批开始----审批流编号:"+processInstanceId);
                }else if("finish".equals(type)){
                    log.info("----审批关闭----审批流编号:"+processInstanceId);
                    String result = eventJson.getString("result");
                    if ("agree".equals(result)){
                        log.info("----审批完成----审批流编号:"+processInstanceId);
                        //流程完成所以结束
                        this.shenhe(processInstanceId);
                    } else {
                        log.info("----审批拒绝----审批流编号:"+processInstanceId);
                        //修改标注位
                        this.shenheshibai(processInstanceId);
                    }
                }else if ("terminate".equals(type)) {
                    log.info("----审批撤回----审批流编号:"+processInstanceId);
                    this.shenheshibai(processInstanceId);
                }
            }
            // 5. 返回success的加密数据
